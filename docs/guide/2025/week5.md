...
本题考查了 Node.js 中的「原型链污染」漏洞。
</Container>

尝试找出服务中存在的各个模块中，可能造成原型链污染的方法，并且上网搜寻其相关内容。

当我们在对大多数服务<span data-desc>（特别是 Go、Node.js、Python 等强模块相关的语言）</span>进行渗透时，通常需要了解该服务中所利用到的模块的版本信息，并且根据模块版本信息来上网搜寻相关的 CVE，或者查询 Github 上的历史 commit 中被修改掉的内容，很可能就是解题的关键。
